...
FaceShield 的算法任务是图片级伪造人脸二分类。系统使用阶段只输入一张待检测图片，经过人脸裁剪和图像预处理后，模型输出 real/fake 二分类结果、伪造概率和可解释性热力图；业务展示层再根据伪造概率生成风险等级。
...
其中 `label` 仍然是模型二分类结果，默认按 `fake_probability >= 0.50`
映射为 fake，否则为 real。`risk_level` 只用于业务提示和前端展示，
不是训练标签，也不改变二分类评估口径。
