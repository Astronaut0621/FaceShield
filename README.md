...
- 后端：FastAPI + SQLAlchemy，支持 SQLite 开发库和 openGauss 兼容 SQL。
...
database/   openGauss 建表 SQL、schema 和数据库说明
...
默认使用 SQLite：`backend/faceshield.db`。如需连接 openGauss，设置 `DATABASE_URL`：
